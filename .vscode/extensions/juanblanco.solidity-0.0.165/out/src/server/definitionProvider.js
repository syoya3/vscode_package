...
exports.SolidityDefinitionProviderOld = exports.SolidityDefinitionProvider = exports.SolidityReferencesProvider = exports.SolidityHoverProvider = void 0;
...
class SolidityDefinitionProviderOld {
    ...
    provideDefinition(document, position) {
        ...
        if (this.project !== undefined) {
            contracts.addSourceDocumentAndResolveImports(contractPath, documentText, this.project);
        }
        // this contract
        ...
        if (element !== undefined) {
            switch (element.type) {
                case 'ImportStatement':
                    return Promise.resolve(vscode.Location.create(vscode_uri_1.URI.file(this.resolveImportPath(element.from, contract)).toString(), vscode.Range.create(0, 0, 0, 0)));
                case 'ContractStatement': {
                    // find definition for inheritance
                    const isBlock = this.findElementByOffset(element.is, offset);
                    if (isBlock !== undefined) {
                        let directImport = this.findDirectImport(document, result.body, isBlock.name, 'ContractStatement', contracts);
                        if (directImport.location === undefined) {
                            directImport = this.findDirectImport(document, result.body, isBlock.name, 'InterfaceStatement', contracts);
                        }
    ...
    provideDefinitionForVariable(contracts, name) {
        return this.provideDefinitionForContractMember(contracts, name, (element) => element.body.filter(contractElement => contractElement.name === name && (contractElement.type === 'StateVariableDeclaration')));
    }
    /**
     ...
     * @memberof SolidityDefinitionProvider
     */
    provideDefinitionForContractMember(contracts, literalFallbackName, extractElements) {
        const locations = [];
        for (const contract of contracts.documents) {
            const uri = vscode_uri_1.URI.file(contract.absolutePath).toString();
            ...
            try {
                ...
                if (pos > -1) {
                    ...
                    if (pos > -1) { // making sure..
                        locations.push(vscode.Location.create(uri, vscode.Range.create(document.positionAt(pos), document.positionAt(pos + literalFallbackName.length))));
                    }
                }
            }
        }
    ...
    replaceCommentsWithSpacesPreservingLines(code) {
    ...
    provideDefinitionForType(document, documentStatements, literal, contracts) {
        if (literal.members.length > 0) {
            ...
            const enumLocation = this.findStatementLocationByNameType(document, contractStatement.body, literal.literal, 'EnumDeclaration');
            if (enumLocation !== undefined) {
                return Promise.resolve(enumLocation);
            }
            // TODO: only search inheritance chain
            return this.provideDefinitionForContractMember(contracts, literal.literal, (element) => element.body.filter(contractElement => contractElement.name === literal.literal && (contractElement.type === 'StructDeclaration' || contractElement.type === 'EnumDeclaration')));
        }
    }
    /**
    ...
    findDirectImport(document, statements, name, type, contracts) {
        ...
        for (let i = 0; location === undefined && i < contract.imports.length; i++) {
            ...
            const importContract = contracts.documents.find(e => e.absolutePath === importPath);
            const uri = vscode_uri_1.URI.file(importContract.absolutePath).toString();
    ...
    resolveImportPath(importPath, contract) {
        if (contract.isImportLocal(importPath)) {
            return contract.formatDocumentPath(path.resolve(path.dirname(contract.absolutePath), importPath));
        }
...
exports.SolidityDefinitionProviderOld = SolidityDefinitionProviderOld;
